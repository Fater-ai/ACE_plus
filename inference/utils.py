...
class ACEPlusImageProcessor():
    ...
    def preprocess(self,
        ...
        actual_scale_w = rW / W if W > 0 else 0.0 # Avoid division by zero if W is somehow 0
        slice_w = math.ceil(slice_w * actual_scale_w)

        edit_image = T.Resize((rH, rW), interpolation=T.InterpolationMode.NEAREST_EXACT, antialias=False)(edit_image)
        edit_mask = T.Resize((rH, rW), interpolation=T.InterpolationMode.NEAREST_EXACT, antialias=False)(edit_mask)
